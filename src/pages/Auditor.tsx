          ...
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-bold text-slate-800">
                {tab === 'workspace' ? 'Audit Workspace' : tab === 'ncr' ? 'Non-Conformance Reports' : tab === 'fraud' ? 'Fraud Log' : tab === 'flags' ? 'System Flags' : 'Auditor Console'}
              </h1>
               ...
               <div className="p-5 space-y-3">
                 {allFlags.filter(f => f.rule_id !== 'AUDIT-NCR').map(flag => (
                   <div key={flag.id} className="bg-slate-50 border border-slate-200 rounded-lg p-4 flex gap-4 items-start">
                     ...
                     <div className="flex-1">
                       <div className="flex justify-between items-start mb-1">
                         <div className="text-xs font-mono font-bold text-slate-600">{flag.rule_id} • Batch {flag.batch_id_label || flag.record_id}</div>
                       ...
                       <p className="text-sm text-slate-700">Flag generated by automated rule Engine.</p>
